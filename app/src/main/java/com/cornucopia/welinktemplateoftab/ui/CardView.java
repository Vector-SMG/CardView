...
import com.cornucopia.welinktemplateoftab.widget.recyclerview.MaxHeightRecyclerView;
...
public class CardView extends FrameLayout {
  ...
  private void initNotifyView() {
    ...
    final LooperLayoutManger looper = new LooperLayoutManger();
    looper.setLooperEnable(true);
    looper.setAutoMeasureEnabled(true);
    rv.setLayoutManager(looper);

    //模拟通知数据
    final List<NotifyItemBean> list = new ArrayList<>();
    ...
    notifyItemBean.setValue(
        "1-5号结账期间收入，回款展示预估数据(非财经定稿),上月核算数据将于5号24:00定稿发布;1-5号结账期间收入，回款展示预估数据(非财经定稿),上月核算数据将于5号24:00定稿发布");
    ...
    NotifyRecyclerViewAdapter notifyRecyclerViewAdapter =
        new NotifyRecyclerViewAdapter(this.context,
            R.layout.welink_uibundle_business_mould_notify_item_layout, list);
    ...
    rv.getViewTreeObserver().addOnGlobalLayoutListener(
        new ViewTreeObserver.OnGlobalLayoutListener() {
          @Override public void onGlobalLayout() {
            ...
            Log.e(TAG, "onGlobalLayout start:" + System.currentTimeMillis());

            for (int i = 0; i < list.size(); i++) {
              LinearLayout ll = (LinearLayout) rv.getLayoutManager().findViewByPosition(i);
              Log.e(TAG, ll + "");
            }
            Log.e(TAG, "onGlobalLayout end:" + System.currentTimeMillis());
            rv.setmMaxHeight(UiUtils.dpToPx(context, 72));
            //延迟3s启动通知列表的滚动
  ...
  private void initTemplatesView() {
    ...
    titles.add("xxxxxxxxxxxxxxxxxx");
    titles.add("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
    titles.add("12345678910");
    ...
    mTabContainerLayout.setMode(TabContainerLayout.MODE_SCROLLABLE);
  }
